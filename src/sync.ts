...
async function updateFromSourceSpec(
    ...
    try {
        ...
        const doesBranchExist = await branchExists(
            owner,
            repo,
            branch,
            octokit,
        ...
        await exec.exec(
            "git",
            ["commit", "-m", "Update API specifications with fern api update"],
            { silent: true },
        );
...
async function updateTargetSpec(
    token: string,
    branch: string,
    autoMerge: boolean,
): Promise<void> {
    ...
    for (const [index, mapping] of fileMapping.entries()) {
        if (!mapping.from || !mapping.to) {
            throw new Error(
                `File mapping at index ${index} is missing required 'from' or 'to' field`,
            );
        }
    }
...
async function cloneRepository(options: SyncOptions): Promise<void> {
    ...
    try {
        ...
        if (error instanceof Error) {
            throw new Error(
                `Failed to verify repository access: ${error.message}`,
            );
        } else {
            throw new Error(
                "An unknown error occurred while verifying repository access",
            );
        }
    }
...
async function syncChanges(options: SyncOptions): Promise<void> {
    ...
    try {
        ...
        if (options.autoMerge) {
            core.info(
                `Auto-merge enabled. Will push directly to branch: ${workingBranch}`,
            );
        } else {
            core.info(
                `Auto-merge disabled. Will create PR from branch: ${workingBranch} to main`,
            );
        }
...
async function commitChanges(): Promise<void> {
    ...
    await exec.exec(
        "git",
        ["commit", "-m", `Sync OpenAPI files from ${github.context.repo.repo}`],
        { silent: true },
    );
}
